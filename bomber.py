...
try:
    ...
    print('[!] Ошибка: некоторые зависимости не установлены')
    print('Type \'pip install -r requirements.txt\' to install all required packages')
    exit()

colors=['\033[1;31m','\033[1;32m','\033[1;33m','\033[1;34m','\033[1;35m','\033[1;36m']
W='\033[0m'
# Преимущество этого кода принадлежит DeadTX и всем другим авторам, перечисленным на https://github.com/DeadTeX/SkyBomb
# Если вы хотите взять кредит на этот код, пожалуйста, посмотрите сами
...
def infinite(pn, dl, ch, max):
    ...
    while True:
        ...
        count_inf += 1
        # os.system('echo DeadTX >> count.xxx')
        time.sleep(float(dl))
        if (count_inf > maxlim):
            exit()
...
def checkinternet():
    ...
    try:
        ...
        res = True
    if res:
        print("\n\n\tКажется, у вас низкая скорость интернета или вы используете прокси ...")
        print('\t\tSkyBomb Остановится сейчас...\n\n')
        banner()
        exit()
...
def start(target, counter, delay, ch, cc):
    ...
    while success < (int(bombs)):
        ...
        try:
            ...
            if cc == "91":
                print('Извините, все API истекли, пожалуйста, обновите SkyBomb')
                input('Нажмите Enter Чтоб Выйти...')
                exit()
            else:
                if success > 0:
                    print(
                        '\n\n\tМы сожалеем, что достигнут предел бомбардировок в вашей стране...')
                    print(
                        '\nМы слишком усердно работаем, чтобы увеличить международный лимит...')
                    input(
                        '\nЭто поможет нам оказать поддержку вашей стране...\n\nНажми Enter Чтоб Выйти...')
                    os.system('rm *.xxx* > /dev/null 2>&1')
                    print('\n\n')
                    banner()
                    exit()
                else:
                    print('\n\n\tИзвините, ваша страна не поддерживается...')
                    print(
                        '\t\tПожалуста По Всем Вопросам В Telegram:https://t.me/nonamehacks...')
                    input('Нажми Enter Чтоб Выйти...')
                    exit()
        print(random.choice(colors))
        print("==================================================================")
        print("               Бомбардировка продолжается, пожалуйста, подождите !!               ")
        print("     Пожалуйста, сохраняйте соединение для передачи данных активным во время бомбежки!    ")
        print("==================================================================")
        print("             Введи КоД Страны        : +" + str(cc) + " ", target)
        print("             Введи Номер  Телефона Sent : ", requested)
        print("             Кол-во Флуда      : ", success)
        print("             Неудачные запросы         : ",failed)
        print("==================================================================")
        print("            Скрипт  этот для удовольствия, а не для мести !!                ")
        print("              Этот бомбардировщик был создан DeadTX!!                ")
        print("==================================================================")
        try:
...
def update():
    ...
    for fl in stuff_to_update:
        dat = urllib.request.urlopen(
            "https://raw.githubusercontent.com/DeadTeX/SkyBomb/master/SkyBomb.sh" + fl).read()
        file = open(fl, 'wb')
        file.write(dat)
        file.close()
    print('\n\t\tUpdated Successfull !!!!')
...
try:
    ...
    print("Вы Не Подключенны К Интернету!!!")
    print("\tПодключи Интернет,попробуй Снова...\n")
    input('Выходим....\nНажми Enter Чтоб Выйти....')
    exit()
print('\tПроверяем На Надичие Обновлений...')
ver = urllib.request.urlopen(
    "https://raw.githubusercontent.com/DeadTeX/SkyBomb/master/.version").read().decode('utf-8')
verl = ''
try:
...
try:
    noti = urllib.request.urlopen(
        "https://raw.githubusercontent.com/DeadTeX/SkyBomb/master/.notify").read().decode('utf-8')
...
while True:
    pn = ""
    cc = input("\tВведите код страны (без +) : ")
    if '+' in cc:
        tc = list(cc)
        tc.remove('+')
        cc = ''.join(tc)
        cc = cc.strip()
    pn = input("\tВведите номер цели: +" + cc + " ")
    pn = remsp(pn)
    if len(cc) >= 4 or len(cc) < 1:
        print('\n\n Неверный код страны..\n\t\tКоды стран обычно состоит из 1-3 цифр...\n')
        continue
    if len(pn) <= 6:
...
try:
    ...
    type = 0
if type == 1:
    nm = int(input("Введите количество звонков для отправки (максимум 15): "))
    if nm > 15:
        print("\t\tВы вошли " + str(nm) +
              ".\n\tНормализация значения до 15")
        nm = 15
    dl = float(input("Введите время задержки (в секундах) [Рекомендуется 10 секунд  ] : "))
elif type == 0:
    if cc == "91":
        nm = int(input("Введите количество сообщений для отправки (0 для неограниченного: "))
        dl = float(
            input("Введите время задержки (в секундах) [Рекомендуется 2 секунды] : "))
    else:
        nm = int(input("Введите количество сообщений для отправки: "))
        dl = float(
            input("Введите время задержки (в секундах) [Рекомендуется 2 секунды]: "))
maxlim = 0
if cc == "91":
    ...
    maxlim = 100
if nm > maxlim:
    print('\n\n\tИзвините, из-за неправильного использования этого скрипта мы предоставляем только ' +
          str(maxlim) + ' СМС сразу...\n\n')
    print('Количество SMS было установлено на ' + str(maxlim))
    nm = maxlim
if not cc.strip() == "91":
    if type == 1:
        print(
            '\t\tИзвините, но Бомбардировка по вызову в настоящее время поддерживается только для индийских номеров !!!!')
        print()
        input('Нажми Enter Чтоб Выйти....')
        print('\n\n')
        banner()
    ...
    if pn.strip() == '' or dl <= 0 or nm <= 0 or cc.strip() == '' or cc.find('+') != -1:
        print('\n\n\t Похоже, вы дали неправильные данные...')
        input('\n\t\tНажми Enter Чтоб Выйти...')
        banner()
        exit()
    ch = [0, 14, 15, 16]
...
if pn.strip() == '' or dl <= 0 or nm < 0:
    ...
    exit()
if type == 1:
    print("ПРИМЕЧАНИЕ: Call Bomb может не работать на активированных номерах DND...\n")
    print("\n\tПожалуйста, не перегружайте Call Bomb, чтобы он работал в течение более длительного периода времени...")
    cbomb = True
if cbomb:
    ...
    exit()
if nm == 0:
    nt = int(input("\tКоличество потоков (от 10 до 20): "))
    if nt <= 0 or nt >= 30:
        print('\tSkyBomb показывает лучший результат от 10 до 25 потоков\n\t\tВсе еще продолжается....')
    print("\n\nПожалуйста, помните, что это в экспериментальной стадии и невероятно быстро ...")
    t = [None] * nt
    print(random.choice(colors))
    print("\n\n==================================================================")
    print("                Готовясь к бомбардировке, пожалуйста, подождите !!               ")
    print("     Пожалуйста, сохраняйте подключение к данным во время бомбардировки!!    ")
    print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    print("             Целевой номер       : +91", pn)
    print("             Количество потоков  : ", nt)
    print("             задержка               : ", dl)
    print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    print("              Использовать Этот Скрипт Только Для Шутки !!                ")
    print("              Этот Бомбер Создан DeadTX !!                ")
    print(" .             Связаться Со Мной Можно По Телеграму:https://t.me/nonamehacks")
    print("___________________________________________________________________")
    print(W)
    input('\n\nНажми CTRL+Z Чтоб Остановить Бомбежку... \nНажми Enter Чтоб Запустить Бомбер...\n')
    os.system('rm *.xxx* > /dev/null 2>&1')
    print("\n\nStarting Bomb....")
    for i in range(nt):
    ...
    while True:
        ci += 1
        l = count_inf
        print("    Общее количество отправленных запросов : ", l)
        if int(l) > maxlim:
            print('\n\n\tИзвините, из-за неправильного использования этого скрипта мы предоставляем только ' +
